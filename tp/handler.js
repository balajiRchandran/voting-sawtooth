...
class VotingSystemHandler extends TransactionHandler {
    ...
    async handleAddPartyTransaction(context, payload) {
        ...
        //console.log("Party Id"+payload.partyId);
        if (partyExists) {
    ...
    async handleAddVoteTransaction(context, payload) {
        ...
        var endstring = '2021-04-30T23:59:59.00';
        var enddate = new Date(endstring);

        var difference = enddate - curdate; // difference in milliseconds
        if(difference < 0)
        {
            throw new InvalidTransaction(`Voting Process ended by `+endstring);
        }
